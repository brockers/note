...
- docfile: [claude/ai_docs/typescript_specific.md]
...
```typescript
Examples:
 - JSON schemas and avj-ts for validation
...
Task 6: CREATE _tests/{component}.test.tsx
  - IMPLEMENT: Jest/Testing Library tests for components and hooks
  - FOLLOW pattern: _tests/existing.test.tsx (test structure, mocking patterns)
...
npm test -- _tests/{domain}.test.tsx
npm test -- _tests/use{Hook}.test.ts
